...
class Generator extends \yii\gii\Generator
{
    ...
    public function getViewPath()
    {
        if (empty($this->viewPath)) {
            ...
            $alias = Yii::getAlias($this->viewPath);
            return $alias;
        }
    }
    ...
    public function generateActiveField($attribute)
    {
        ...
        if ($column->phpType === 'boolean' || preg_match('/^(is_active)$/i', $attribute)) {
            return "\$form->field(\$model, '$attribute')->checkbox()";
        } elseif ($column->phpType === 'integer' || preg_match('/^(\w_id)$/i', $attribute)) {
            ...
            if (is_array($column->enumValues) && count($column->enumValues) > 0) {
                ...
                return "\$form->field(\$model, '$attribute')->$input(['maxlength' => 255])";
            }
        }
    }
